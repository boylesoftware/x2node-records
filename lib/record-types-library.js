...
class RecordTypesLibrary {
	 ...
	 * library.
	 *
	 * @param {(string|Symbol)} recordTypeName Record type name. Using a
	 ...
	 * @returns {module:x2node-records~RecordTypeDescriptor} Descriptor of the
	 * new record type.
	...
	addRecordType(recordTypeName, recordTypeDef) {
		...
		this._finalizers = new Array();
		const desc = new RecordTypeDescriptor(
			this, recordTypeName, recordTypeDef);
		this._recordTypeDescs[recordTypeName] = desc;
