...
class RecordTypesLibraryFactoryContext {
	...
	completeLibrary() {
		...
		this._onLibraryCompleteHandlersPlan.ctxHandlers =
			this._onLibraryCompleteHandlersPlan.byLibrary;
		for (let handlers of this._onLibraryCompleteHandlersPlan.byProperties)
			for (let handler of handlers)
				handler(this._recordTypes);
		for (let handlers of this._onLibraryCompleteHandlersPlan.byContainers)
			for (let handler of handlers)
				handler(this._recordTypes);
		for (let handler of this._onLibraryCompleteHandlersPlan.byLibrary)
			handler(this._recordTypes);
		this._completingLibrary = false;
	}
	...
	completeContainer(container) {
		...
		for (let handlers of handlersPlan.byProperties)
			for (let handler of handlers)
				handler(container);
		for (let handler of handlersPlan.byContainer)
			handler(container);
