...
class CoreExtention {
	...
	extendPropertyDescriptor(ctx, propDesc) {
		...
		// determine if modifiable
		if (!propDesc.isView() && (propDef.modifiable !== undefined))
			propDesc._modifiable = (propDef.modifiable ? true : false);
		else
			propDesc._modifiable = (!propDesc._isId && !propDesc.isView());
		ctx.onLibraryValidation(() => {
			if (propDesc.isModifiable() &&
				(propDesc.isId() || propDesc.isView()))
		...
		/**
		 * Tell if the property value is modifiable. Note, that being not
		 * modifiable does not necessarily mean unmutable. It merely means that
		 * setting a new value directly to the property is disallowed. By
		 * default, the core does not allow id and view properties to be
		 * modifiable.
		 *
		 * @function module:x2node-records~PropertyDescriptor#isModifiable
		 * @returns {boolean} <code>true</code> if the property is modifiable.
		 */
		propDesc.isModifiable = function() { return this._modifiable; };
		...
		/**
		 * Tell if the property is polymorphic reference. Note, that the
		...
		/**
		 * Tell if the property is polymorphic nested object or reference. Either
		 * way, the <code>scalarValueType</code> of such property is "object".
		 *
		 ...
		 * polymorphic nested object or reference container. The
		 * <code>scalarValueType</code> for such property can be either "object"
		 * or "ref", it is always scalar and never polymorphic.
		 *
		 ...
		 * <code>scalarValueType</code> is always "object".
		 *
		 * @function module:x2node-records~PropertyDescriptor#isRef
		 * @returns {boolean} <code>true</code> if the property is a
		 * non-polymorphic reference.
		 */
		propDesc.isRef =
			function() { return (this._scalarValueType === 'ref'); };

		/**
		 * Name of the target record type for a non-polymorphic reference
		 * property.
		 *
		 ...
		 * record type names.
		 *
		 * @member {Array.<string>=} module:x2node-records~PropertyDescriptor#subtypes
		 * @readonly
		 */
